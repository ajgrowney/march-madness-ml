...
teams_df = pd.read_csv(os.path.join(COMPETITION_SRC, "MTeams.csv")).drop(columns=['FirstD1Season', 'LastD1Season'])
model = pickle.load(open('model_2019.sav', 'rb'))
...
if "userin" in sys.argv:
    ...
    ncaa_tourney_games = pd.read_csv(os.path.join(COMPETITION_SRC, "MNCAATourneyCompactResults.csv"))
